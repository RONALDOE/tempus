...
function UpdateEmployee() {
  ...
  const _idEmployee = location.pathname.split("/")[2]
  useEffect(() => {
  async function fetchData() {
    const res = await axios.get(`http://localhost:8000/employees/${_idEmployee}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try{
        await axios.post(`http://localhost:8000/employees/${_idEmployee}`, employee)
  ...
  return (
    <form onSubmit={handleSubmit}  noValidate className="updateForm">
      <label className="labelUpdate" htmlFor="name">Name:</label>
      <input
        type="text"
        id="name"
        ...
        name="_name"
        placeholder={employeeData._name}
        required
      />
      <p className="error">{nameError}</p>
      <br />
      <label className="labelUpdate" htmlFor="lastname">Lastname:</label>
      <input
        type="text"
        className="textUpdate"
        id="lastname"
        name="_lastName"
        onChange={handleChange}
        placeholder={employeeData._lastName}
        required
      />
      <p className="error">{lastNameError}</p>

      <br />
      <label className="labelUpdate" htmlFor="idnumber">ID Number:</label>
      <InputMask
        mask='999-9999999-9'
        className="textUpdate"
        type="text"
        ...
        placeholder={employeeData._idNumber}
        required
      />
      <p className="error">{idNumberError}</p>

      <br />
      <label className="labelUpdate" htmlFor="cellphone">Cellphone:</label>
      <InputMask
        mask='(999)-999-9999'
        className="textUpdate"
        type="text"
        ...
        name="_cellphone"
        onChange={handleChangeMasked}
        placeholder={employeeData._cellphone}
        required
      />
      <p className="error">{cellphoneError}</p>

      <br />
      <label className="labelUpdate" htmlFor="email">Email:</label>
      <input
        className="emailInsert"
        type="text"
        id="email"
        name="_email"
        placeholder={employeeData._email}
        onChange={handleChange}
        required
      />
      <p className="error">{emailError}</p>

      <button className="buttonUpdate" type="submit">Create</button>
